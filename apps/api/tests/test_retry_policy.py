...
def test_capcut_task_timeout_is_retryable():
    mapped = map_provider_error(CapCutTaskError("TTS Task timed out after 90 seconds"))
...
def test_download_connection_error_is_retryable():
    ...
    mapped = map_download_error(httpx.ConnectError("connection reset", request=request))
